...
export default function ActiveReviewPlatformsStrip({
  ...
  if (platforms.length === 0) {
    return (
      <p className={["text-xs text-slate-600", className].filter(Boolean).join(" ")}>
  ...
  return (
    ...
    >
      <span className="text-[11px] font-semibold uppercase tracking-wide text-slate-600">Review links on</span>
      <ul className="flex flex-wrap items-center gap-2">
        {platforms.map((p) => (
          <li
            key={p.id}
            className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-medium text-slate-800"
          >
            <ReviewPlatformIcon platformId={p.id} size="sm" />
            <span>
              {p.label}
              {p.storeCount > 1 ? <span className="text-slate-600 font-normal"> ({p.storeCount} stores)</span> : null}
            </span>
          </li>
        ))}
      </ul>
