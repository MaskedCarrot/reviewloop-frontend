...
export default function Logo({
  ...
  const mark = (
    <span className={`inline-flex items-center justify-center ${dim} rounded-lg bg-brand-600 text-white`}>
  ...
  return (
    <Link href={href} className="flex items-center gap-2.5">
      {mark}
      <span className={`font-semibold tracking-tight text-slate-900 ${text}`}>ReviewLoop</span>
    </Link>
  );
}
